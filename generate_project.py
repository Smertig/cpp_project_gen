...
parser.add_argument("--unique-header-lines", action="store_true", help="generate unique lines in each header")
...
UNIQUE_HEADER_LINES = args.unique_header_lines
...
def get_header_template(i: int) -> str:
    def get_suffix(j: int) -> str:
        return f"_{j}" if UNIQUE_HEADER_LINES and j > 0 else ""

    function_declaration = (f"void foo_{i}{get_suffix(j)}(int a, int b);" for j in range(NUMBER_OF_LINES_IN_HEADER))
